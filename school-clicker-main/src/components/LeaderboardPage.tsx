...
export const LeaderboardPage: React.FC<LeaderboardPageProps> = ({ isMuted, onToggleMute }) => {
  ...
  const handleRegionChange = (region: string) => {
    ...
    // Don't clear search when changing regions - let universal search work
         ...
         if (selectedSchool) {
           ...
           const shouldAutoScroll = region === "ALL" || (selectedSchoolData && selectedSchoolData.region === region);
  ...
  const handleSubmitRequest = async () => {
    if (schoolName.trim() && schoolLocation.trim() && userEmail.trim()) {
      try {
        console.log("Submitting school request:", { name: schoolName, location: schoolLocation, email: userEmail });
        
        // Submit to Firebase
        const requestId = await submitSchoolRequest(schoolName, schoolLocation, userEmail);
      ...
      alert("Please fill in all fields (school name, location, and email).");
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="w-full max-w-2xl mb-2 px-4">
          <div className="bg-gradient-to-r from-purple-600/20 via-pink-600/20 to-purple-600/20 backdrop-blur-sm border border-purple-500/50 rounded-lg p-2 md:p-2.5 shadow-xl">
            <div className="flex items-center justify-center gap-1.5 md:gap-2 flex-wrap">
              {/* Days */}
              <div className="flex flex-col items-center bg-black/40 rounded px-2 py-1 md:px-2.5 md:py-1 min-w-[50px] md:min-w-[65px]">
                <div className="font-pixelify font-bold text-xl md:text-2xl text-white">
                  {countdown.days.toString().padStart(2, '0')}
                </div>
                <div className="font-pixelify font-normal text-[9px] md:text-[10px] text-purple-300 uppercase">
                  Days
                </div>
              </div>

              <div className="font-pixelify font-bold text-lg md:text-xl text-purple-400">:</div>
              ...
              {/* Seconds */}
              <div className="flex flex-col items-center bg-black/40 rounded px-2 py-1 md:px-2.5 md:py-1 min-w-[50px] md:min-w-[65px]">
                <div className="font-pixelify font-bold text-xl md:text-2xl text-white animate-pulse">
        ...
        <div className="w-full max-w-4xl border-[3px] border-solid border-white bg-black flex-1 flex flex-col min-h-0">
          ...
          <div className="w-full p-3 border-b border-solid border-white/30 flex items-center opacity-80">
           ...
           {/* Region Tabs */}
           <div className="w-full p-2 border-b-2 border-solid border-white flex items-center justify-center gap-2">
             {["ALL", "NY", "CA", "IL", "TOP"].map((region) => {
               ...
               const shouldTriggerAutoScroll = selectedSchool && (
                 region === "ALL" || 
                 (selectedSchoolData && selectedSchoolData.region === region)
               );
          ...
          <button 
            ...
            className="w-full p-2 md:p-4 bg-[#2600A3] flex items-center justify-center font-pingfang font-normal text-white text-xs md:text-sm lg:text-base tracking-[0] leading-[normal] hover:bg-[#1a0080] transition-colors"
          >
